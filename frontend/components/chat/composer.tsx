  ...
  return (
    <div className={cn("fixed bottom-4 left-0 right-0 px-4 pointer-events-none z-10", hasAnimated && "composer-intro")}>
      <div className="relative max-w-2xl mx-auto pointer-events-auto">
        <div
          className={cn(
            "relative flex flex-col gap-3 overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-950/95 p-4 transition-all duration-200",
            "focus-within:border-zinc-600 focus-within:ring-2 focus-within:ring-white/10",
          )}
          style={{
            boxShadow:
              "rgba(0, 0, 0, 0.32) 0px 0px 0px 1px, rgba(0, 0, 0, 0.18) 0px 1px 1px -0.5px, rgba(0, 0, 0, 0.16) 0px 3px 3px -1.5px, rgba(0, 0, 0, 0.14) 0px 6px 6px -3px, rgba(0, 0, 0, 0.12) 0px 12px 12px -6px, rgba(0, 0, 0, 0.1) 0px 24px 24px -12px",
          }}
        >
          <div className="flex gap-2 items-center">
            {uploadedImage && (
              <div className={cn("relative shrink-0", showImageBounce && "image-bounce")}>
                <div className="w-12 h-12 overflow-hidden rounded-lg border border-zinc-800">
                  <Image
                ...
                <button
                  onClick={removeImage}
                  className="absolute -top-1.5 -right-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-zinc-200 text-zinc-950 transition-colors hover:bg-white"
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              disabled={isStreaming || disabled}
              size="icon"
                className="h-9 w-9 shrink-0 rounded-full bg-white/10 text-zinc-100 hover:bg-white/15"
            ...
            <span className="text-xs text-zinc-400">{currentModel.name}</span>
          </div>
        </div>
      </div>
    </div>
